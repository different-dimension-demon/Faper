    ...
    def train(self, dataset, batch_size):
        ...
        for idx in tqdm(range(len(dataset)//batch_size), desc='Training epoch ' + str(self.epoch + 1) + ''):
            ...
            for batch_idx in range(0, batch_size):
                ...
                join_feat = feature[:, self.operat_dim : self.operat_dim + self.join_dim]
                table_feat = feature[:, self.operat_dim + self.join_dim : self.operat_dim + self.join_dim + self.table_dim ]
                filter_feat = feature[:, self.operat_dim + self.join_dim + self.table_dim : self.operat_dim + self.join_dim + self.table_dim + self.filter_dim]
    ...
    def test(self, dataset, itr):
        ...
        with torch.no_grad():
            ...
            for idx in tqdm(range(0, len(dataset))):
                ...
                for m in mean:
                    total_final_estimate_output.append(m)
                for l in data['labels']:
                    total_final_label.append(l)
